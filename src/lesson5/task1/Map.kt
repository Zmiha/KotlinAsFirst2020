...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((i, k) in grades) {
        results.getOrPut(k) { mutableListOf() }.add(i)
    }
    return results
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    for ((key, value) in stuff) {
        if ((value.first == kind) && (value.second.toInt() < minPrice)) {
            minPrice = value.second.toInt()
            result = key
        }
    }
    return if (result == "qwe") null else result
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    if (list.isNotEmpty()) {
        Loop@ for (i in list) {
            for (k in list) {
                if ((i + k == number) && (list.indexOf(i) != list.lastIndexOf(k))) {
                    result = Pair(list.indexOf(i), list.lastIndexOf(k))
                    break@Loop
                }
            }
        }
